...
public class Bot extends Player {
    ...
    public void moveBot(boolean[][] map1, int playerX, int playerY, int botX, int botY) {
        ...
        map[botY / GameLoader.cellSize][botX / GameLoader.cellSize] = false;

        Vector2D from, to;
        from = new Vector2D(playerY / GameLoader.cellSize, playerX / GameLoader.cellSize);
        to = new Vector2D(cords.y / GameLoader.cellSize, cords.x / GameLoader.cellSize);

        ArrayList<Vector2D> path;

        path = LeeAlgorithm.findPath(map, to, from);
        if (path != null) {
